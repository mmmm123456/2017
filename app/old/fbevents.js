/*1487167126,,JIT Construction: v2835688,en_US*/
    ...
    (function(a, b, c, d) {
        ...
        var e = "2.5.1",
            ...
            o = function(ua) {
                ...
                ya.prototype = {
                    append: function ab(bb, cb) {
                        this._append(encodeURIComponent(bb), cb, wa);
                    },
                    _append: function ab(bb, cb, db) {
                        if (Object(cb) !== cb) {
                            this._appendPrimitive(bb, cb);
                        } else if (db === wa) {
                            this._appendObject(bb, cb);
                        } else this._appendPrimitive(bb, za(cb));
                    },
                    _appendPrimitive: function ab(bb, cb) {
                        if (cb != null) this.list.push([bb, cb]);
                    },
                    _appendObject: function ab(bb, cb) {
                        for (var db in cb)
                            if (cb.hasOwnProperty(db)) {
                                var eb = bb + '[' + encodeURIComponent(db) + ']';
                                this._append(eb, cb[db], xa);
                            }
                    },
                    each: function ab(bb) {
                        var cb = this.list;
            ...
            q = function(ua) {
                ...
                eb.prototype = {
                    validateMetadata: function hb() {
                        var ib = this.metadata.toLowerCase(),
                            jb = cb[ib];
                        if (!jb) return this._error('Unsupported metadata argument: ' + ib);
                        return this;
                    },
                    validateEvent: function hb() {
                        ...
                        for (var lb in kb)
                            if (db.call(kb, lb)) {
                                var mb = kb[lb];
                                if (mb.isRequired === true && !db.call(this.params, lb)) return this._error('Required parameter "' + lb + '" is missing for event "' + ib + '"');
                                if (mb.type)
                                    if (!this._validateParam(lb, mb.type)) return this._error('Parameter "' + lb + '" is invalid for event "' + ib + '"');
                            }
                        return this;
                    },
                    _validateParam: function hb(ib, jb) {
                        ...
                        switch (jb) {
                            case xa:
                                var lb = wa.test(kb);
                                if (lb && Number(kb) < 0) this.warnings.push('Parameter "' + ib + '" is negative for event "' + this.eventName + '"');
                                return lb;
                            case ya:
                ...
                function fb(hb) {
                    return new eb(null, null, hb).validateMetadata();
                }

                function gb(hb, ib) {
                    return new eb(hb, ib).validateEvent();
                }
                ua.validateMetadata = fb;
                ua.validateEvent = gb;
                return va.exports;
            }({}),
            r = null,
        ...
        function fa(ua) {
            if (!j.length) {
                ...
                switch (ua) {
                    case 'addPixelId':
                        w = true;
                        return ga.apply(this, va);
                    case 'init':
                        x = true;
                        return ga.apply(this, va);
                    case 'set':
                        return ha.apply(this, va);
                    case 'track':
                        if (m.test(va[0])) return ka.apply(this, va);
                        if (wa) return ja.apply(this, va);
                        return ia.apply(this, va);
                    case 'trackCustom':
                        return ja.apply(this, va);
                    case 'send':
                        return ma.apply(this, va);
                    default:
        ...
        function ia(ua, va) {
            ...
            if (ua === 'CustomEvent') ua = va.event;
            ja.call(this, ua, va);
        }

        function ja(ua, va) {
            var wa = this instanceof ea ? this : n,
                xa = ua === 'PageView';
        ...
        (function ua() {
            ...
            var va = function wa() {
                ...
                fa.call(xa, 'trackCustom', 'PageView');
            };
