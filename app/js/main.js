function init() {
    for (container = document.getElementById("particles"), canvas = document.createElement("canvas"), ctx = canvas.getContext("2d"), man = !1, tog = !0, list = [], w = canvas.width = COLS * SPACING + 2 * MARGIN, h = canvas.height = ROWS * SPACING + 2 * MARGIN, i = 0; i < NUM_PARTICLES; i++) p = Object.create(particle), p.x = p.ox = MARGIN + SPACING * (i % COLS), p.y = p.oy = MARGIN + SPACING * Math.floor(i / COLS), list[i] = p;
    container.addEventListener("mousemove", function(e) {
        bounds = container.getBoundingClientRect(), mx = e.clientX - bounds.left, my = e.clientY - bounds.top, man = !0
    }), "function" == typeof Stats && document.body.appendChild((stats = new Stats).domElement), container.appendChild(canvas)
}

function step() {
    if (stats && stats.begin(), tog = !tog)
        for (man || (t = .001 * +new Date, mx = .5 * w + Math.cos(2.1 * t) * Math.cos(.9 * t) * w * .45, my = .5 * h + Math.sin(3.2 * t) * Math.tan(Math.sin(.8 * t)) * h * .45), i = 0; i < NUM_PARTICLES; i++) p = list[i], d = (dx = mx - p.x) * dx + (dy = my - p.y) * dy, f = -THICKNESS / d, d < THICKNESS && (t = Math.atan2(dy, dx), p.vx += f * Math.cos(t), p.vy += f * Math.sin(t)), p.x += (p.vx *= DRAG) + (p.ox - p.x) * EASE, p.y += (p.vy *= DRAG) + (p.oy - p.y) * EASE;
    else {
        for (b = (a = ctx.createImageData(w, h)).data, i = 0; i < NUM_PARTICLES; i++) p = list[i], b[n = 4 * (~~p.x + ~~p.y * w)] = b[n + 1] = b[n + 2] = COLOR, b[n + 3] = 255;
        ctx.putImageData(a, 0, 0)
    }
    stats && stats.end(), requestAnimationFrame(step)
}

function ajustVideo($video) {
    var aspectRatio = 1920 / 1080,
        newW = $(window).width(),
        newH = $(window).height(),
        newAspect = newW / newH;
    if (aspectRatio > newAspect) var vidW = "300%",
        vidH = "100%",
        top = 0,
        left = "-100%";
    else var vidW = "100%",
...
function randomWord() {
    if (!blocked) return count >= total && (count = 0), randWord = words[count], $moldura.removeClass("--active"), $gifs.css("background-image", ""), $elem.css("opacity", 1), $btClick.text(textBt), clearTimeout(intervaloWords), autoChangeWords(), count++, $("#home").find(".frase strong").text(randWord)
}

function randomBt() {
    var textBt = wordsBt[Math.floor(Math.random() * wordsBt.length)];
    $btClick.text(textBt)
}

function showgif() {
    if (!blocked) {
        var img = baseUrl + "img/home/" + randWord + ".gif";
        $moldura.addClass("--active"), $elem.css("opacity", 0), $gifs.css("background-image", "url(" + img + ")"), randomBt(), clearTimeout(intervaloWords)
    }
}

function autoChangeWords() {
    intervaloWords = setTimeout(randomWord, 200)
}

function loadedHome() {
    blocked = !1, $loading.addClass("out"), setTimeout(function() {
        randomWord(), $textLoading.addClass("out"), $elem.filter(".center").removeClass("center"), $overlay.removeClass("--flat"), $home.find(".hide").removeClass("hide").addClass("animated")
    }, 500)
}

function calcLoaded() {
    loadedPercent = Math.round(loaded / total * 100), $loadingText.text(loadedPercent), TweenMax.to($loadingBarra, .5, {
        width: loadedPercent + "%"
    })
}
...
var $about, $txtAbout1, $txtAbout2, loadedAbout = !1,
    outAbout = !1,
    scrolling = !1,
    ...
    about = function(ajax) {
        if ($about = $("#about"), outAbout = !1, $about.length > 0) {
            ...
            ajustVideo($video), $headerAbout.css("margin-bottom", wHeight), $contato.css("margin-bottom", wHeight), $colR.height(wHeight), $(window).resize(function() {
...
about(), $(function() {
    Modernizr.svg || $(".cliente__img img").each(function() {
        var srcSvg = $(this).attr("src").split("/")[2],
            srcPng = srcSvg.split(".")[0];
        $(this).attr("src", "img/clientes/" + srcPng + ".png")
    })
});
var words = ["awesome", "bold", "cuckoo", "curious", "intense", "loud", "slick", "fearless", "unique"],
    ...
    loaded = 0,
    loadedPercent = 0,
    randWord, $textLoading, $loading, $loadingBarra, $loadingText, $home, $moldura, $btClick, $elem, $overlay, intervaloWords, $gifs;
$(document).ready(function() {
    $home = $("#home"), $gifs = $home.find(".gifs-landing"), $moldura = $home.find(".moldura-landing"), $btClick = $home.find(".clickgif"), $loading = $home.find(".barra-loading"), $loadingBarra = $home.find(".barra-loading__barra"), $loadingText = $home.find(".barra-loading__porcentagem"), $textLoading = $home.find(".text-loading"), $elem = $home.find(".elem"), $overlay = $home.find(".overlay-landing"), loadImages(), $home.on("mousedown touchstart", ".link-landing", function(event) {
        event.stopPropagation()
    }), Modernizr.touch ? ($home.on("touchstart", showgif), $home.on("touchend", randomWord)) : ($home.on("mousedown", showgif), $home.on("mouseup", randomWord), $home.on("mouseenter", ".link-landing", function() {
    ...
    }), $home.on("mouseleave", ".link-landing", function() {
        $moldura.find(".moldura-landing__left,.moldura-landing__right").removeClass("--active")
    ...
    $("body").on("submit", ".form-sing-up", function() {
        var form = $(this),
            ...
            email = $campo.val();
        return "" == email && (flag = !1, $campo.focus()), 1 == flag ? $.ajax({
            type: "POST",
            url: baseUrl + "paginas/cadastra-newsletter.php",
            data: form.serialize(),
            success: function(data) {
                "true" == data ? ($campo.removeClass("--error"), $msg.filter(".--error").removeClass("--show"), $msg.filter(".--success").addClass("--show"), $(form)[0].reset()) : "erro-campos" == data ? $campo.addClass("--error") : "erro-duplicado" == data ? ($campo.addClass("--error"), $msg.filter(".--error").addClass("--show"), $(form)[0].reset()) : "erro-inserir" == data && ($campo.addClass("--error"), $msg.filter(".--error").addClass("--show"))
            },
            error: function(errorThrown) {
                console.log(errorThrown)
            }
        }) : ($msg.filter(".--error").addClass("--show"), $campo.addClass("--error")), !1
    })
});
...
works(), $(document).ready(function() {
    var currentPage, loadedPages = [],
        ...
        loadPage = function(url, back) {
            ...
            return nextPage != currentPage && ($.ajax({
                method: "GET",
                url: url
            }).done(function(data) {
                back || urlChange(url);
        ...
        getIdPage = function(url) {
            var hash = window.location.hash,
                id = url.replace(baseUrl + idioma + "/", "").replace(baseUrl, "").replace("page/", "").replace(hash, "").replace(/\/$/, "").split("/");
    ...
    loadedPages.push(pageInit), currentPage = pageInit, $("body").on("click", ".link-page", function(event) {
        ...
        if ("#contato" == hash) {
            ...
            $(window).scrollTop(0), TweenMax.to(window, 2, {
                scrollTo: top
            })
        }
